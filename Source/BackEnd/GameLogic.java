...
public class GameLogic {
    ...
    Phase phase;
    //List of CarColours for later storage
    ArrayList<CarColours> colourList = new ArrayList<>();

    //Stores the controller for colour setting
    GameScreenController theController;
     ...
     * @return Returns an empty list if the board is random, and returns the list of objects if it isn't for further processing (Displaying car colours etc.)
	 */
	public ArrayList<Object> newGame(String boardFile, GameSave gameSaver, HashMap<String, String> initData, GameScreenController controller) throws Exception {
		doubleMove = false;
		currentPlayerNo = 0;
		phase = DRAW;
		numberOfPlayers = 4;
		ArrayList<Object> gameItems = FileReader.gameSetup(boardFile, seed, initData);
		gameboard = (Gameboard) gameItems.get(0);
		players = (Player[]) gameItems.get(1);



		currentPlayer = players[currentPlayerNo];
		this.gameSaver = gameSaver;
		if ((boolean) gameItems.get(2))   {
            controller.setCarImage((CarColours) gameItems.get(3));

            for(int i = 3; i<=6; i++)   {
                colourList.add(((CarColours) gameItems.get(i)));
            }
		    return gameItems;
        }else   {
		    controller.setCarImage(CarColours.PINK);
		    colourList.add(CarColours.PINK);
		    colourList.add(CarColours.YELLOW);
		    colourList.add(CarColours.TURQUOISE);
		    colourList.add(CarColours.ORANGE);
     ...
     */
    public void move(Coordinate location, GameScreenController theController) throws Exception {
        ...
        if (gameboard.isPlayerOnGoal() != -1) {
            ...
            phase = DRAW;
            theController.setCarImage(colourList.get((currentPlayerNo + 1) % getNumberOfPlayers()));
     ...
     */
    public void saveGame(GameScreenController colorTransfer) throws IOException {
        gameSaver.saveToFile(colorTransfer);
    }
