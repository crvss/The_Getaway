...
public class GameLoad {
	 ...
	 */
	public static Pair<GameLogic, Profile[]> loader(HashMap<String, String> initData, GameScreenController gameController) throws Exception {
		...
		gameLogic.newGame(gameBoard, gameSave, initData,gameController);
		...
		while (in.hasNextLine()) {
			...
			switch (choiceType) {
				...
				case "move":
					x = lineReader.nextInt();
					y = lineReader.nextInt();
					gameLogic.move(new Coordinate(x, y), gameController);
					break;
				case "COLOURS":
					if (gameController != null)	{
						ArrayList<CarColours> colorList = new ArrayList<>();
						colorList.add(CarColours.valueOf(in.nextLine()));
						colorList.add(CarColours.valueOf(in.nextLine()));
						colorList.add(CarColours.valueOf(in.nextLine()));
						colorList.add(CarColours.valueOf(in.nextLine()));
						gameController.setCarColours(colorList);
					}
			}
		}

		//I'm not sure why the loader gets called twice, but this if is here to ensure that colours are only set if it's called from the controller
