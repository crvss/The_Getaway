...
public class GameSave {
     ...
     * @param colorTransfer
     */
    public void saveToFile(GameScreenController colorTransfer) throws IOException {
        FileWriter writer = new FileWriter(gameSaveFile, true);
        writer.write(gameState.toString());
        ArrayList<CarColours> colorsToSave = colorTransfer.getCarColours();
        writer.write("COLOURS");
        writer.write(System.lineSeparator());
        for(CarColours pc:colorsToSave)    {
            writer.write(String.valueOf(pc));
            writer.write(System.lineSeparator());
        }
        writer.flush();
        writer.close();
        isGameSaved = true;
        CustomAlerts.Warning("User Alert!", "File " + gameSaveFile.getName() + " has been saved", true);
    }
    /**
