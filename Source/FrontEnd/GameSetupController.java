...
public class GameSetupController extends StateLoad {
		...
		if (selectGameBoard.getValue() == null) {
			...
			if (gameBoards != null) {
				for (String gameBoard : gameBoards) {
					...
					if (getInitData() != null) {
						getInitData().put("LevelType", "game");
					}
				}
			}
	...
	public void onStartButton() {
		...
		if ((gameSaveName.equals(""))) {
			...
			CustomAlerts.Warning("User Alert!", "You must give your game a name!", true);
			return;
		}
		...
		if (gameSaveFile.exists()) {
			...
			saveName.setStyle("-fx-prompt-text-fill:red;");
			CustomAlerts.Warning("User Alert!", "The name " + gameSaveName + " is already in use!", true);
			return;
		}
